...
# Total session cost (prefix of line 2)
COST_STR=$(awk "BEGIN {printf \"%.2f\", ${TOTAL_COST:-0}}")
COST_DISPLAY="💰 ${C_GREEN}\$${COST_STR}${RESET} | "
...
# Format 5H reset as clock time "16:49"
format_reset_clock() {
    local reset_epoch="$1"
    [[ -z "$reset_epoch" || "$reset_epoch" == "null" ]] && return
...
if [[ -n "$FIVE_HOUR_PCT" ]]; then
    ...
    FIVE_RESET_CLOCK=$(format_reset_clock "$FIVE_HOUR_RESET")
    ...
    FIVE_DISPLAY="${C_LAVENDER}5H${RESET} ${FIVE_BAR} ${BOLD}\033[38;2;${FIVE_END_COLOR}m${FIVE_HOUR}%${RESET} (${FIVE_RESET_CLOCK}[${FIVE_RESET_FMT}])"
    SEVEN_DISPLAY="${C_YELLOW}7D${RESET} ${SEVEN_BAR} ${BOLD}\033[38;2;${SEVEN_END_COLOR}m${SEVEN_DAY}%${RESET} (${SEVEN_RESET_FMT})"

    LINE2="${COST_DISPLAY}${CTX_DISPLAY} │ ${FIVE_DISPLAY} │ ${SEVEN_DISPLAY}"
else
    ...
    LINE2="${COST_DISPLAY}${CTX_DISPLAY} │ ${FIVE_DISPLAY} │ ${SEVEN_DISPLAY} ${C_OVERLAY}(loading..)${RESET}"
fi
